...
def setup_dependency_logging(entry_id: str) -> None:
    ...
    try:
        ...
        try:
            # Remove Loguru's process-wide default console sink so dependency
            # logs do not bypass Home Assistant logging.
...
def _remove_loguru_sink() -> None:
    """Tear down the Loguru sink to avoid leaking on integration unload."""
    global _loguru_sink_id
    if _loguru_sink_id is not None:
        try:
